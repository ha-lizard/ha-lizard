...
# Wait for the Power Management menu and check if the server is powered off
expect {
    timeout { puts "Error: Timeout waiting for Power Management menu"; exit 1 }
    eof { puts "Error: SSH connection failed"; exit 1 }
    "Power Status : Off" {
      puts "Info: Server is already powered off";
      exit 1
    }
    "(1) Immediate Power Off" { send "1" }
}

# Wait for the confirmation prompt to confirm power-off action
expect {
    timeout { puts "Error: Timeout waiting for power-off confirmation"; exit 1 }
    eof { puts "Error: SSH connection failed"; exit 1 }
...
puts "Info: Server has been powered OFF successfully."
exit 0
